...
#define CMD_3 "13" //전력사용량 (앞은 led 번호) 03은 led0, 13은 led1
...
//    Serial.print("sector : ");
    switch(sector_rx[1])
    {
      case '0' : //led 1에서는 A sector이지만 led2에서는 Bsector pin
        sector_on_off[1] = true; //sector 제어를 표시   
...
void Passing()
{
  ...
  if(running_night == true)
  {
    ...
    while(passing_again)
    {
      ...
      {
         if(sector_on_off[0] == true ) //sector B제어 중
         {
      ...
      else if(sector_on_off[1] == true) //sector A 제어 중일 경우
      {
           start_time[0] = millis();
           temp_time = start_time[0] + waiting_time_first;
           During_On(temp_time);
